...
public class GuiStorageMonitor extends GuiContainer
{
	...
	public GuiStorageMonitor(InventoryPlayer player, TEStorageMonitor tile)
	{
		...
		for (int i = 0; i < buttons.length; i++)
		{
			...
			buttons[i] = new CButton(i, 0, 0, 24, 13, 1, 192, 1, 207, displayStrings[i % 4], 0xFFFFFF, 0xFFFFAF, ChargingBench.proxy.GUI3_PNG);
		}
	}
	...
	{
		// Draw tier and title
		fontRenderer.drawString("Storage Monitor", 54, 12, 4210752);

		if (tile.energyStored == -1 || tile.energyCapacity == -1)
		{
			Utils.drawCenteredText(fontRenderer, "No Valid Link", 90, 35, 0xFF5555);
		}
		else
		{
			// Draw Right-aligned current energy number
			Utils.drawRightAlignedText(fontRenderer, Integer.toString(tile.energyStored), 90, 35, 0x55FF55);

			// Draw separator and left-aligned max energy number
			fontRenderer.drawString(" / " + Integer.toString(tile.energyCapacity), 90, 35, 0x55FF55);
...
//			fontRenderer.drawString(" / " + "123456789", 85, 21, 4210752);
		...
		fontRenderer.drawString("Upper Threshold (Off)", 41, 49, 0xA03333);
		Utils.drawRightAlignedText(fontRenderer, upper, 109, 63, 0x55FF55);
		
		fontRenderer.drawString("Lower Threshold (On)", 41, 78, 0xA03333);
		Utils.drawRightAlignedText(fontRenderer, lower, 109, 92, 0x55FF55);
	}
	...
	protected void drawGuiContainerBackgroundLayer(float var1, int mouseX, int mouseY)
	{
		...
		final int horizOffs[] = {-57, -33, 25, 49};
		for (int i = 0; i < 8; i++)
		{
